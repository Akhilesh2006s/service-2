  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (userType === "organization") {
      ...
      if (dataMode === "fake") {
        ...
        toast.success(`Welcome to Inkaranya, ${signUpData.companyName}! (Fake Data)`);
        navigate("/organizations");
        return;
      }
      // Live mode: Firestore
      try {
        ...
        localStorage.setItem("userEmail", signUpData.email);
        localStorage.setItem("companyName", signUpData.companyName);
        toast.success(`Welcome to Inkaranya, ${signUpData.companyName}!`);
        navigate("/organizations");
      } catch (err) {
      ...
      if (dataMode === "fake") {
        ...
        toast.success(`Welcome to Inkaranya, ${signUpData.fullName}! (Fake Data)`);
        navigate("/employees");
        return;
      }
      // Live mode: Firestore
      try {
        ...
        localStorage.setItem("userPortfolio", signUpData.portfolio || "");
        localStorage.setItem(`applications:${signUpData.email}`, JSON.stringify([]));
        toast.success(`Welcome to Inkaranya, ${signUpData.fullName}!`);
        navigate("/employees");
      } catch (err) {
      ...
      <div className="w-full max-w-md">
        ...
        <Card className="bg-black/80 border-yellow-500/30 backdrop-blur-sm">
          <CardHeader className="text-center">
            ...
            <CardTitle className="text-2xl text-yellow-400">Inkaranya</CardTitle>
